...
def plot_correlations(data: pd.DataFrame) -> None:
    ...
    plt.figure()
    correlation_matrix.total_cases.drop("total_cases").sort_values(
        ascending=False
    ).plot.barh()
...
def distribution_plots(data: pd.DataFrame) -> None:
    num_cols = len(data.columns)
    num_rows = (num_cols + 1) // 3

    _, axes = plt.subplots(nrows=num_rows, ncols=3, figsize=(15, 3 * num_rows))
    axes = axes.flatten()
